    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-neutral-950">
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-amber-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero?.badge || 'Distinguished Collection'}
        </motion.p>

        <div className="overflow-hidden">
          {['EMINENT', 'DRIVES', 'AWAIT.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1
                className={`font-heading leading-[0.88] tracking-tight ${line === 'DRIVES' ? 'bg-gradient-to-r from-orange-500 via-orange-400 to-orange-600 bg-clip-text text-transparent' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'DRIVES' ? 900 : 300 }}
              >
                {line}
              </h1>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }} className="flex items-center gap-3 mt-8">
          ...
          <p className="text-white/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            32 Reviews &middot; 4.6 Stars &middot; Prince Edward St, Harare
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.3 }} className="flex items-center gap-3 mt-6">
          <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-4 py-2">
            ...
            <span className="text-amber-400 text-xs uppercase tracking-[0.15em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>Finance Available | Trade-Ins Welcome</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.5 }} className="flex flex-wrap gap-4 mt-8">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-amber-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-amber-400 hover:shadow-xl hover:shadow-amber-500/20" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero?.ctaPrimary || 'Browse Vehicles'}
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/projects" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-amber-500/50 hover:text-amber-400 hover:bg-white/5" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-amber-500/80 font-heading text-lg sm:text-2xl tracking-wider font-bold uppercase">{item}</span>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Transformations</p>
              <h2 className="font-heading text-white leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                Before <span className="text-amber-500">&</span> After
              </h2>
            </div>
    ...
    <section ref={ref} className="bg-neutral-950 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Expertise</p>
              <h2 className="font-heading text-white leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                What We <span className="text-amber-500">Repair</span>
              </h2>
            </div>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-amber-500/15 font-heading text-6xl sm:text-7xl font-bold leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats?.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="text-center relative">
              <div className="font-heading text-amber-500 leading-none font-bold" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)', textShadow: '0 0 40px rgba(249,115,22,0.15)' }}>
              ...
              {i < (stats?.length || 0) - 1 && <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-amber-500/15 to-transparent" />}
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-amber-500 mb-6" />
            <p className="text-amber-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Legacy</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              Eminent <span className="text-amber-500">Panel Beaters</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Founded on a commitment to factory-grade vehicle restoration, Eminent Cars has earned the trust of Harare's motorists through relentless attention to detail and uncompromising quality standards.
            </p>
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl font-bold leading-none">4.2</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Star Rating</div>
              </div>
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl font-bold leading-none">32</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Reviews</div>
              </div>
              <div>
                <div className="text-amber-500 font-heading text-3xl sm:text-4xl font-bold leading-none">12mo</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 sm:-right-6 bg-amber-500 text-neutral-950 p-5 sm:p-7 shadow-2xl">
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-amber-500">Eminent</span>
            </h2>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-0.5 mt-1">{[...Array(t.rating || 5)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-amber-500" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-amber-500 hover:border-amber-500/30 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">{testimonials.map((_, i) => <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-amber-500' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i + 1}`} />)}</div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-amber-500 hover:border-amber-500/30 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <section ref={ref} className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative z-10 max-w-3xl mx-auto px-5 sm:px-8 text-center">
        ...
        <h2 className="font-heading text-white leading-[0.92] font-bold uppercase mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
          Your Vehicle Deserves <span className="text-amber-500">Precision</span>
        </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/contact" className="group inline-flex items-center justify-center gap-3 bg-amber-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-amber-400 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
            Browse Vehicles <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <a href="https://wa.me/263772981406" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 border border-green-500/40 text-green-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-green-500/10 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
